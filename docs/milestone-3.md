...
and metrics are free and everything else, including all admin and write
routes, is denied.

- `scripts/seed-firn.sh` plus `config/seed.json`: a one-shot compose service
  that waits for Firn, upserts 8 text rows (with small placeholder vectors,
  since Firn requires a vector payload per row; BM25 carries the demo), then
  builds the FTS index and polls the async operation to completion.
- `config/rules.json`: the new pricing table. Only the query route costs
...
No gateway code changed. Monetising a new origin endpoint was a rules-table
edit, which is what the design is for.
...
Codex's headline finding was real: prefix-priced routes plus downstream URL
normalisation meant `POST /firn/ns/demo/query/../upsert` would have been
priced as a query and delivered to Firn as an admin write. The gateway now
rejects paths with dot segments (plain or percent-encoded) or empty
segments, and it does so before pricing, so nobody pays for a request that
will be refused. Verified with `curl --path-as-is` (400); reqwest-based
clients normalise client-side and never even send it. Also applied: the
seed script tolerates reruns and ends with a probe query as its acceptance
test, operation-id parsing cannot poll garbage, and the client's hit
printer no longer slices mid-UTF-8.
...
- **Firn rejects text-only rows.** Every row needs a vector payload (the
  ...
  workload. Small 4-dimension placeholder vectors satisfy the schema.
- **FTS is a build step, not a default.** Queries with `text` fail with
  ...
  created" until you `POST /ns/{ns}/fts-index` and poll the async
  operation.
- **reqwest-middleware's builder is not reqwest's.** x402-reqwest returns a
  ...
  you enable an extra feature on a crate this repo does not otherwise
  depend on; setting the content-type and body by hand avoids the
  dependency.
- A stale detail from milestone 2: mounted config is read at startup, so
  editing `rules.json` needs a container restart (`--force-recreate` after
  a rules change, or the old table keeps pricing).
