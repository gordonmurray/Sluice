...
- `gateway/` — axum service with two routes: `GET /healthz` (free, answered by
  the gateway) and `GET /firn/health` (paid: 0.01 USDC on Base, x402-axum layer,
  reverse-proxied to the Firn origin with the `/firn` prefix stripped). The
  gateway never touches the chain; verify/settle happens in the facilitator.
- `client/` — x402-reqwest test client proving the loop: free 200 → bare 402
  with payment requirements → signed EIP-3009 retry → 200 + settlement header.
- `docker-compose.yml` — rung 1 stack: anvil (forking Base mainnet), one-shot
  `fund-usdc`, the prebuilt x402 facilitator, MinIO + Firn (built from the
  local `../firnflow` checkout), and the gateway.
...
- `scripts/cargo.sh` — dockerized cargo wrapper (see "surprises").
...
for this repo (`0xa749...43DA`) — see the EIP-7702 surprise below for why it
...
request bodies, fix stale account docs. Deferred: the `/firn` prefix
...
rung-1 keys to `.env` (they are documented fake-value dev keys; testnet
...
- **Anvil's publicly known dev accounts are unusable as x402 payers on a Base
  mainnet fork.** Their keys are public, so people have installed EIP-7702
  delegations on them on the real chain (`cast code` shows `0xef0100...`).
  Forked USDC's SignatureChecker then treats them as ERC-1271 contract wallets
  and rejects plain ECDSA payment authorizations with "FiatTokenV2: invalid
  signature" — even though the signature is provably correct. The client uses a
  fresh keypair generated for this repo instead. Dev accounts are still fine as
  facilitator signer (sends normal txs) and pay-to (only receives).
- **A forked anvil's clock freezes at the fork block.** EIP-3009 authorizations
  carry `validAfter` ≈ wall clock, so verification fails with "authorization is
  not yet valid" once the fork is minutes old. Fixed with `--block-time 5` so
  chain time tracks wall clock.

- **The host had no usable Rust toolchain.** rustup/cargo installed fine, but
  there is no C toolchain (`cc`/`ld`) on the host and no passwordless sudo, so
  cargo cannot link binaries. All builds run inside the `rust:1.96-bookworm`
  image via `scripts/cargo.sh` (registry cache in a named volume, incremental
  target/ on the bind mount). CLAUDE.md's "nothing beyond docker and cargo"
  effectively became "nothing beyond docker".
- **The facilitator's README config format is ahead of its released image.**
  The 2.0.1 image wants `{"id": "...", "chains": "eip155:8453"}` (string
  pattern), not the `{"scheme": ..., "chains": [...]}` shown in the README on
  main.
- **x402-chain-eip155 2.0.1 does not compile without its `telemetry` feature**
  (it references `tracing` outside the feature gate), so the feature is on.
- **x402-reqwest 2.0.1 is built against reqwest 0.13** — a 0.12 workspace pin
  ...
  `rustls`.
- CLAUDE.md's `X402Middleware::try_from(<facilitator_url>)` note is accurate
  for 2.0.1 — no change needed there.
