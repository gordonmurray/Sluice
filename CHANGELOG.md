...
- Settlement recording with an operator-driven refund policy: the origin's outcome (`origin_status`) is recorded to the payments table so refunds can be made out of band, as an explicit operator decision rather than an automatic gateway action. No automatic retry, no automatic credit, so the gateway stays stateless. The chain is the source of truth for settlements; receipt recording is best-effort, and a crash mid-request can drop one.
